...
class ConstantLengthDataset(IterableDataset):
    ...
    def batch_to_device(self, batch, device, keep_label=True):
        inputs = batch['input_ids']
        if isinstance(inputs, dict):
            for feature_name in inputs:
                inputs[feature_name] = inputs[feature_name].to(device)
        else:
        ...
        if not keep_label:
            ...
            labels = torch.LongTensor(label_space).to(device)
    ...
    def __iter__(self):
        ...
        while more_examples:
            ...
            for example in examples:
                ...
                inputs, labels = self.batch_to_device(batch, self.visible_devices)
                yield {
                        "input_ids": inputs,
                        "labels": labels,
                    }
