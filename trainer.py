...
class CollateFunctionIter():
    def __init__(self, up_model):
        self.up_model = up_model

    def __call__(self, batch):
        if isinstance(batch, dict):
            batch = [batch] # extend to batch size of 1
            padded_features = self.up_model.padding_features(batch)
...
class ModelTrainer():
    def __init__(self, up_model: nn.Module, down_layer: nn.Module = None, train_dataset=None,
        ...
        self.collate_fn = CollateFunctionIter(self.up_model)
        ...
        '''
        if n_gpu > 1:
            self.model = torch.nn.DataParallel(self.model, device_ids=[int(i) for i in visible_devices.split(',')])
        ...
        print("total  train  steps:", self._total_train_steps)
        self._epochs = epochs
    ...
    def train(self):
        ...
        logger.info(
            f"  Num of training examples (actually iterations per epoch for Iterable Dataset) = {30}")
        logger.info(f"  Output path (short): {self.output_path}")
        logger.info(
